...
(let ()
  (define obj (let ((o (Object)))
                ...
                o))
  (assert (obj.getValue) 42)              ; direct: method auto-bound to obj

  (define getter obj.getValue)
  (assert (getter) 42)                    ; survives being stored in a var

  (define (fetch o) o.getValue)
  (assert ((fetch obj)) 42)               ; survives being a function RESULT

  (define arr (vector 3 1 2))
  (define joiner arr.join)
  (assert (joiner "-") "3-1-2")           ; native this-dependent method

  (define sorter arr.sort)
  (sorter (lambda (a b) (- a b)))       ; LIPS lambda crossing into native
  (assert arr #(1 2 3))
  (assert (arr.map (lambda (x) (* x x))) #(1 4 9))

  (assert (eq? arr.push.valueOf Function.prototype.valueOf) #t))  ; auto-unbind
